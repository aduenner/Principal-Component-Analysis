...
if __name__ == "__main__":
    ...
    # training_data = np.array(training_data.reshape(len(training_data), -1), dtype="uint8")
    # test_data = np.array(test_data.reshape(len(test_data), -1), dtype="uint8")
    ...
    with torch.cuda.device(1):
        ...
        for val in [5.0, 10.0, 20.0, 50.0]:

            noise_images_training = np.load("noised_data_training_" + str(int(val)) + ".npy").reshape(-1, 1, 28, 28)
            noise_images_test = np.load("noised_data_test_" + str(int(val)) + ".npy").reshape(-1, 1, 28, 28)
            ...
            eval_(noise_images_test, test_labels, n_classes, n_dims, class_names, "model_params.pt")
