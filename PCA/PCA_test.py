...
comlist = [1,2,3,5,10,50,100,300,768]
image_set = np.load('NNHelper/Set1/speckled_data_test_0.01.npy')/1.
image_set=image_set[0:1000,:]
for i in range(9):
    set, comps = pca_transform(image_set, comlist[i], 'Full_SVD')
    shape = int(np.ceil(np.sqrt(np.shape(set)[1])))
    iout = extract_image(set,shape,0)
    plt.subplot(3,3,i+1)
    plt.imshow(iout,cmap='gray')
    plt.axis('off')
    plt.title(repr(comlist[i])+' PCs')
plt.show()
