...
'''
Note ~90% of operation time is spent performing the dot operations:
    p = np.dot(X.T, t) and
    t = np.dot(X, p)
np.dot is already implemented using a BLAS backend, which is already highly optimized. 
Utilizing Numba on top of it does not result in much speedup
'''
@jit(nopython=True)
